...
- [вышла новая версия Safari 18.2](https://webkit.org/blog/16301/webkit-features-in-safari-18-2/):
  ...
  - добавлена функция Scroll to [Text Fragments](https://developer.mozilla.org/en-US/docs/Web/URI/Fragment/Text_fragments)
  - поддержан `<input type=week>`
  - у [`PointerEvent`](https://developer.mozilla.org/en-US/docs/Web/API/PointerEvent) появились методы `getPredictedEvents()` и `getCoalescedEvents()` для предсказания будущих позиций курсора и получения нескольких сгруппированных `PointerEvent` в `pointermove`
  - поддержан [`Promise.try`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise/try)
...
- CSSWG подытоживают год и выкатывают 4 новых FPWD: [Display 4](https://drafts.csswg.org/css-display-4/), [Overflow 5](https://drafts.csswg.org/css-overflow-5/), [Multicol 2](https://drafts.csswg.org/css-multicol-2/) и [Color HDR 1](https://drafts.csswg.org/css-color-hdr/), из интересных нововведений:
  - [reading-flow](https://drafts.csswg.org/css-display-4/#propdef-reading-flow) для управления порядком чтения контента скринридером в flex- и grid-контейнере
  - интерполяция и анимация свойства `display`
  - псевдоэлемент `::column` в многоколоночном контейнере
  - свойство `scroll-marker-group` и группа псевдоэлементов [`::scroll-marker-*`](https://drafts.csswg.org/css-overflow-5/#scroll-navigation) для стилизации скролл-контейнера
  - управление яркостью HDR-контента
- [вышел отчёт Cloudflare Radar 2024](https://radar.cloudflare.com/year-in-review/2024):
...
- порой бывает нужно сделать небольшой проект, куда тянуть React не обязательно, и в таком случае не обязательно тянуть и [бандлер вместе с процессом билда](https://playfulprogramming.com/posts/modern-js-bundleless):
  - для локального сервера и HMR сгодится `browser-sync`, сторонние пакеты можно загрузить с  [unpkg.com](https://unpkg.com/) или из npm и прописать алиасы в `<script type="importmap">`
