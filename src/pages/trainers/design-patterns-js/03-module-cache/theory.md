# Паттерн Module (Модуль), практика: кэш с WeakMap
...
function processSurveyData(obj) {
  ...
  const result = Object.values(obj).map(calculateAverage);
...
Тогда при повторном вызове функции `processSurveyData` результат не будет вычислять заново, а ранее вычисленное значение просто возьмётся из кэша, и функция отработает быстрее.
...
В нашем случае после выполнения вычислений внутри `processSurveyData` результат перед возвращением нужно записать в кэш:

```js
cache.set(obj, result);
```

А перед вычислением проверять наличие уже вычисленного значения в кэше, и в случае его наличия, сразу же возвращать:
...
Реализуйте в модуле `module.js` внутри функции `processSurveyData` кеширование вычисления массива средних баллов для каждого пользователя.
