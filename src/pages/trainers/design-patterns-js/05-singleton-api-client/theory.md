...
Представьте, что в приложении есть отдельный модуль `apiClient.js` с классом `ApiClient`. Этот класс служит обёрткой для удобной работы с сетевой библиотекой `axios`. Внутри конструктора класса инициализируется `axios` и инстанс сохраняется в поле `this.client` класса:
...
Также у класса есть метод `get` со строковым параметром `endpoint` (адреса, куда направлять запрос), вызывающий внутри метод API-клиента `this.client.get(endpoint)`:
...
Внутри модуля создаётся инстанс `ApiClient` и экспортирует наружу, но этот инстанс сейчас никак не защищён от модификации. Так что вам нужно применить «фриз» и «заморозить» свежесозданный инстанс:
