...
А некоторые паттерны нативно реализованы в самих JS/React. Например, паттерн `Module` — модульная система встроена в сам язык ES2015, а паттерн `Headless Component` — более фреймворкоспецифичный паттерн, который можно реализовать не только в React, но и в других фреймворках.
