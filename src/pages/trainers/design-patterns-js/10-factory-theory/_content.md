...
А что если требуется создать один класс, который может «производить» объекты разных типов, чтобы в каждом внутри была «зашита» собственная логика/данные? Если решать задачу «в лоб», можно создать «суперкласс», который будет отвечать за всё сразу:

```js
// Плохой подход: вся логика создания внутри одного метода одного класса
...
Более подходящий подход — использовать паттерн Factory (Фабрика). Он представляет собой функцию или метод, принимающий параметры, и на основе параметров создающий объект нужного типа («фабрика» производит объекты):

![Схема паттерна Factory](../assets/scheme-factory-pattern.svg)

Как будет выглядеть класс с таким подходом:
...
Паттерн Factory стоит использовать:

- когда внутри каждого типа «производимого» объекта присутствует разнообразная комплексная логика
- когда все типы объектов заранее неизвестны, предполагается, что будут добавляться новые типы
- когда нужен удобный способ создания разных объектов в зависимости от среды, где выполняется код
- когда нужно реализовать одинаковый «контракт» для создание разных объектов

Паттерн Factory не стоит использовать:
