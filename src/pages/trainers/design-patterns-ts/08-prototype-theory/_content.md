...
Паттерн Prototype в TS реализуется через «нативное» прототипное наследование в языке: одни объекты могут выступать прототипами (шаблонами) для других объектов.
...
Использование «нативного» для TS механизма прототипного наследования заодно благоприятно для перфоманса: функции, объявленные внутри прототипа, не копируются индивидуально в каждый объект, а ссылаются на единственное место в прототипе (то есть занимают меньше места в памяти).
